...
---     if the label is shifted, just like above). This is unacceptable
---     - it looks like the label is for the unlabeled target:
---
---     text        ...xxy|
---     match-1         xL|
---     match-2        xx |
--- 
---     A less problematic variant of this is when `opts.offset_labels`
---     is not set, and a label is on the left of the unlabeled match.
---
--- (C) An unlabeled match covers a label. This is also less
---     problematic, but still unsafe when typing quickly.
--- 
...
function M.resolve_conflicts(targets)
   ...
   local unlabeled_target_at = {}
   local target_with_label_at = {}
   ...
   for _, target in ipairs(targets) do
      ...
      if not is_empty_line then
         ...
         local buf_win_lnum = buf .. ' ' .. win .. ' ' .. lnum .. ' '

         -- We do only one traversal run, and don't assume anything
         -- about the ordering of the targets; a particular conflict
         -- will always be resolved the second time we encounter the
         -- conflicting pair - at that point, one of them will already
         -- have been registered. (From the corresponding pairs of B and
         -- C on the two branches, only one will handle a particular
         -- problem, depending on encountering the unlabeled or the
         -- labeled target later.)

         -- In the examples:
         --   ^^ = current target
         --   capital letters = labeled positions

         if target.label and target.beacon then
            -- We have an active label.
            local label_offset = target.beacon[1]
            local col_label = col_ch1 + label_offset
            local is_shifted_label = col_label == col_ch2
            local other =
               -- Label over label (A)
               --    `xxY|` (offset, win-edge)
               --      ^^
               opts.offset_labels and
               target_with_label_at[buf_win_lnum .. col_label] or
               --
               -- Label touching unlabeled (B1)
               --    `Xxy` (no offset) `xxY|` (offset, win-edge)
               --     ^^                ^^
               unlabeled_target_at[buf_win_lnum .. col_ch1] or
               --
               -- Label over unlabeled (C1)
               --    `xXy` (no offset) `xyXz` or `xxY` (offset)
               --      ^^               ^^        ^^
               unlabeled_target_at[buf_win_lnum .. col_label]
            if other then
               other.beacon = nil
               set_beacon_to_concealed_label(target)
            end
            -- Do this LAST (chasing our own tail is not funny).
            target_with_label_at[buf_win_lnum .. col_label] = target
         else
            -- No label (unlabeled or inactive).
            local col_ch3 = col_ch2 + (target.chars[2]):len()
            local other =
               -- Unlabeled over label (C2)
               --    `xyXz` (offset)
               --       ^^
               opts.offset_labels and
               target_with_label_at[buf_win_lnum .. col_ch1] or
               --
               -- Unlabeled over label (C2).
               --    `xXy` (no offset) `xxY` (offset)
               --     ^^                 ^^
               target_with_label_at[buf_win_lnum .. col_ch2] or
               --
               -- Unlabeled touching label (B2).
               --    `xxY|` (offset, win-edge)
               --     ^^
               opts.offset_labels and
               target_with_label_at[(buf_win_lnum .. col_ch3)]
            if other then
               target.beacon = nil
               set_beacon_to_concealed_label(other)
            end
            unlabeled_target_at[buf_win_lnum .. col_ch1] = target
            unlabeled_target_at[buf_win_lnum .. col_ch2] = target
         end
      end
   end
end
